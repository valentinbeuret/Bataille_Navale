...
char verifier_bateau(size, nb_bateau, matice_A, matrice_B)
{
    ...
    // bateau égale à deux fois 1 donc un bateau est égale à 1
    if (somme_A / 2 < nb_bateau)
    {
...
char verifier_gagnant(size, matice_A, matrice_B, envoyer_bombe)
{
    ...
    for (i=0; i<size; ++i) 
    {
        for (j=0; j<size; ++j) 
        {
            if (matrice_A[i][j] == 2)
            {
                matrice_A[i][j] = 0; // bateau touché donc il en retse toujours un 
                somme_A += matrice_A[i][j];
            }
            else
            {
