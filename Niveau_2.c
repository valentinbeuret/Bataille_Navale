...
void envoyer_bombe(x, y, joueur)
{
    ...
    if(joueur == "A")
    {
        if (matrice_A[x][y] == 1 && matrice_A[x-1][y] == 1 || matrice_A[x+1][y] == 1)
        {
            matrice_A[x][y] = 2;
            printf("Touché");
        }
        if (matrice_A[x-1][y] == 2 || matrice_A[x+1][y] == 2)
        {
            matrice_A[x][y] = 0;
            matrice_A[x-1][y] = 0;
            matrice_A[x+1][y] = 0;
...
char verifier_gagnant(size, matice_A, matrice_B, envoyer_bombe)
{
    ...
    for (i=0; i<size; ++i) 
    {
        for (j=0; j<size; ++j) 
        {
            ...
            // si la partie se termine avec un bateau de toucher donc il y a un 2 soutraire à 1
        }
    }
